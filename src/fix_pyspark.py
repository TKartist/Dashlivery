...
def summarize_df(df):
    ...
    com_numerator = df["Achieved"] + df["Achieved Early"] + df["Achieved Late"]
    com_denominator = df["Achieved"] + df["Not Achieved"] + df["Achieved Early"] + df["Achieved Late"]

    df["Data Completeness"] = np.where(com_denominator != 0, com_numerator / com_denominator, 0)
